    ...
    <div class="row">
        ...
        <form action="{{route('admin.kependudukan.update',$kependudukan->id)}}" method="post">
            ...
            <div class="form-group">
                ...
                @foreach($kependudukans as $kecamatan)
                    <option value="{{$kecamatan->id_kecamatan}}" {{($kecamatan->id_kecamatan == $kependudukan->id_kecamatan)?'selected':''}}>{{$kecamatan->kecamatan}}</option>
                @endforeach
                </select>
